...
var model_hiddenLine =  '<li style="position: relative;border-bottom:1px solid #BABABA;color: black;display: -webkit-box !important;display: box !important;-webkit-box-align: center;box-align: center; padding:0;max-height: 8em;"  <%if(group){%>data-group="<%=group%>"<%}%> <%if(data.id){%>id="<%=data.id%>"<%}%> data-index="<%=index%>">\
<div class="scroll-bar" data-index="<%=index%>" style="width: 100%;z-index:99;overflow:hidden;background-color:#FFF;margin-left: 0px; display: -webkit-box !important;display: box !important;-webkit-box-align: center;box-align: center; max-height: 8em;<%if(data.height){%>height:<%=data.height%>;line-height:<%=data.height%>;<%}else{%>height:100%;line-height:100%;<%}%>">\
<%=data.content%>\
</div>\
<div class="scroll-right ub ub-pc" data-index="<%=index%>" style="position: absolute;top: 0;right: 0;width:4em; color: #FFF;display: -webkit-box !important;display: box !important;-webkit-box-align: center;box-align: center;max-height: 8em;<%if(data.height){%>height:<%=data.height%>;line-height:<%=data.height%>;<%}else{%>height:100%;line-height:100%;<%}%>;background:<%=data.hideOption.style.background%>;"><div style="clear:both;font-size:<%=data.hideOption.style.fontSize%>;">\
        ...
        */
        buildListview : function(data,group) {
                ...
                ele.off('tap').on('tap', function(evt) {
                    if(evt.currentTarget && $(evt.currentTarget)[0].tagName =="LI"){
                       ...
                       // console.log(hasSiblings);
                        if(hasSiblings){
                            $(evt.currentTarget).find(".scroll-bar").css({
                        ...
                        if(data[index].onClick && typeof data[index].onClick == "function"){
                            data[index].onClick($(evt.currentTarget).find(".scroll-bar").children()[0],index);
                        }else{
            ...
            $(document).on("mousedown", ".scroll-bar", function(evt) {
                ...
                delWidth = $(this).siblings(".scroll-right").width();
               // console.log("sssssss",self,hasSiblings)
            });
            ...
            $(document).on("mouseup", ".scroll-bar", function(evt) {
                ...
                //console.log("sdfsdfsd",hasSiblings,startX,moveX,delWidth)
            });
        ...
        add : function(data, dir) {
            ...
            $(".scroll-right").on("tap",function(e){
                ...
                if(data[index].hideOption.onClick && typeof data[index].hideOption.onClick == "function"){
                    data[index].hideOption.onClick($(that).parent(),index,length);
                    e.stopPropagation();
                }
            }); 
            return self;
            ...
            if(uexWindow.getBounce && typeof uexWindow.getBounce === 'function'){
                appcan.window.getBounceStatus(function(dt,dataType,opId){
                    ...
                    $(".scroll-right").on("tap",function(e){
                        ...
                        if(data[index].hideOption.onClick && typeof data[index].hideOption.onClick == "function"){
                            data[index].hideOption.onClick($(that).parent()[0],index,length);
                            e.stopPropagation();
                        }
                    });
                    return self;
